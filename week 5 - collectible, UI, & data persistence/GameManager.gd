...
func _on_signal_bus_coin_changed(current_coin: int) -> void:
	# Mengamankan data koin ke dalam save_game jika komponennya aktif
	if save_game != null:
		save_game.player_coins = current_coin
	print("Koin terdeteksi oleh GameManager: ", current_coin)
